...
function entrada (): number{
    ...
    let n: number = parseFloat(teclado(`Digite número: `)); // não havia o "parseFloat"
    return n; // o "return 2" foi substituído por "return n"
}
...
function menu (): void{
    const teclado = require('prompt-sync')(); // faltavam os dois parênteses finais com o ponto e vírgula 
    ...
    console.log();
    let escolha: string  = teclado(`Digite a opção --> `); // Havia um "parseInt" ao invés de apenas teclado

    opcao(escolha);
}

menu(); // a função não era chamada anteriormente